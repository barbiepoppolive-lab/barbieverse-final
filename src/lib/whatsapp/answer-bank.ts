// The answer bank
// ---------------------------------------------------------------------------
// Every reply below is Barbie's own wording, lifted from her 1,516 messages in
// the 15 Aug 2026 backup, with two deliberate corrections:
//
//   1. Withdrawal is $10 / ~Rs850. She once told a lead "minimum 10000 rupees",
//      which is 12x the real figure and the most discouraging line in 3,545
//      messages.
//   2. The word "guarantee" is gone. She wrote "First month 30k earn krwana
//      meri agency ki guarantee hai" to a real lead — that message is what
//      comes back at you when the lead makes Rs4,000.
//
// Roughly 70-80% of inbound traffic matches one of these, which means most
// replies never touch an LLM: cheaper, instant, and impossible to talk out of.
//
// The app's name never appears. Leads who learn it can install from the Play
// Store directly and we lose both the referral and the agency link.
...
/**
 * Last line of defence before anything is sent — including replies Barbie has
 * edited by hand, which is where a remembered number is most likely to slip in.
...
export function complianceCheck(text: string): { ok: boolean; issues: string[] } {
  ...
  for (const b of BANNED) {
    ...
    // Strip the approved phrasings, then re-test. If the pattern still fires,
    // there's an unapproved number in there and it does not go out.
    let stripped = text;
