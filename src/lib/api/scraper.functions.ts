...
import { importFromFile, getCSVTemplate } from "@/lib/scraper/providers/csv-import";
...
const importToLeadsSchema = z.object({
  jobIds: z.array(z.string()),
});

const deleteJobSchema = z.object({ jobId: z.string() });

const csvTemplateSchema = z.object({
  platform: z.enum(["instagram", "facebook", "twitter", "youtube", "telegram"]),
});

export const getScraperDashboard = createServerFn({ method: "GET" }).handler(async () => {
...
export const runScrapeJob = createServerFn({ method: "POST" })
  .validator(scrapeJobSchema)
  .handler(async ({ data }) => {
    const db = await getDb();
...
export const importResultsToLeads = createServerFn({ method: "POST" })
  .validator(importToLeadsSchema)
  .handler(async ({ data }) => {
    const db = await getDb();
    let imported = 0;
    let skipped = 0;
...
export const getScrapeJobResults = createServerFn({ method: "GET" })
  .validator(z.object({
    jobId: z.string(),
...
export const deleteScrapeJob = createServerFn({ method: "POST" })
...
export const getCSVImportTemplate = createServerFn({ method: "GET" })
  .validator(csvTemplateSchema)
  .handler(async ({ data }) => {
